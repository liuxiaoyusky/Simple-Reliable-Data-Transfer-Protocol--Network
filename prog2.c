...
//https://stackoverflow.com/questions/33110645/warning-implicit-declaration-of-function-is-invalid-in-c99
...
struct msg message;
{
    ...
    seqnum++;
    packet.seqnum = seqnum;//seqnum >= acknum
    ...
    sending = packet;
    tolayer3(0, packet);
    printf("start timer 0, packet %d sending\n", packet.seqnum);
    starttimer(0,15);
    return 1;
}
...
struct pkt packet;
{
    ...
    if (make_checksum(packet) != packet.checksum) {
        printf("%d ACK packet damaged, wait for resent\n", packet.acknum);
        return;
    }

    //advance ACK
    printf("no damage, ACK number +1\n");
    acknum = seqnum;
}
...
struct pkt packet;
{
    ...
    if (make_checksum(packet) != packet.checksum) {
        printf("packet %d damaged, wait for resent\n", packet.seqnum);
        return -1;
    }

    //load data to message and send to layer 5
    struct msg message;
    for (int i = 0; i < 20; i++) {
